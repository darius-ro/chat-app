...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (data.error) {
        // oh no. auth failed or something
        setError(data.error);
      } else {
    ...
    <>
      <div className="flex justify-center items-center text-center h-screen w-screen">
        <div className="bg-neutral-950 rounded-lg border border-neutral-700 p-10 flex w-[90%] max-w-md shadow-lg shadow-black/50 flex-col">
          <div
            className={`bg-red-600 rounded-md p-2 mb-4 ${
              error ? "" : "hidden"
            }`}
          >
          ...
          <input
            className="bg-neutral-950 border border-neutral-700 p-2 my-3 placeholder:text-neutral-600 rounded-md ring-0"
            type="email"
            name="Email"
            id="email"
          ...
          <button
            ...
            type="submit"
            id="login"
            className="mt-6 text-center font-medium py-3 bg-blue-600 hover:bg-blue-700 cursor-pointer transition rounded-md"
          >
            Login
          </button>
          <p className="mt-4 text-neutral-500 text-sm">
            Don't have an account?{" "}
            {/** I have an error on "Don" but uh, i dont know how the apostrophe is related.. */}
            <Link
              href="/register"
